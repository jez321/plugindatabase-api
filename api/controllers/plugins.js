...
const getPlugins = function (req, res) {
  ...
  db.pool.query(`SELECT plugin.id_plugin, plugin.name as name, category.name as category, plugin.created, plugin.updated, company.name as company, link.url as url
                    FROM plugin
                    ...
                    JOIN plugin_link on plugin_link.id_plugin = plugin.id_plugin
                    JOIN link on plugin_link.id_link = link.id_link
                ${searchField}
