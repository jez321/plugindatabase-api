...
const getDeals = function (req, res) {
    ...
    db.pool.query(`SELECT deal.id_deal, plugin.name as name, company.name as company, deal.price, deal.description, deal.start_date, deal.end_date, deal.created as added, json_build_object('title', link.title, 'url', link.url) AS link, t.tags
                FROM deal
