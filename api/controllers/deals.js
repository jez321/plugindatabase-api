...
const getDeals = async function (req, res) {
  ...
  const data = await db.pool.query(`SELECT d.id_deal, d.id_plugin, plugin.name as name, company.name as company, category.name as category, d.price, d.description, d.start_date, d.end_date, d.created as added, json_build_object('title', link.title, 'url', link.url) AS link, t.tags, d.created, d.updated, p_link.url
                FROM deal d
                ...
                JOIN plugin_link on plugin_link.id_plugin = d.id_plugin
                JOIN link as p_link on plugin_link.id_link = p_link.id_link
                ${searchField}
